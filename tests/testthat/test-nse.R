...
test_that("nse", {
  ...
  expect_equal(
    ...
    1 - (sum((ex_dat$obs - ex_dat$pred) ^ 2) / sum((ex_dat$obs - mean(ex_dat$obs)) ^ 2))
  )
  expect_equal(
    nse(ex_dat, truth = obs, estimate = "pred_na", na_rm = TRUE)[[".estimate"]],
    1 - (sum((ex_dat$obs[not_na] - ex_dat$pred[not_na]) ^ 2) / sum((ex_dat$obs[not_na] - mean(ex_dat$obs[not_na])) ^ 2))
  )
})
...
test_that("Result similar to {hydroGOF} package", {
  ...
  expect_equal(
    nse(ex_dat, truth = "obs", estimate = "pred_na", na.rm = TRUE)[[".estimate"]],
...
test_that("Result interpretation is returned", {
  ...
  expect_equal(
    nse(ex_dat, truth = "obs", estimate = "pred", performance = TRUE)[[".estimate"]],
    "Satisfactory"
  )
})
