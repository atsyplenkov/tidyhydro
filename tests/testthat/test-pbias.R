...
test_that("pbias correctly handles missing values", {
  ...
  expect_equal(
    pbias(ex_dat, truth = "obs", estimate = "pred", na.rm = FALSE)[[".estimate"]],
...
test_that("Result similar to {hydroGOF} package", {
  ...
  expect_equal(
    round(pbias(ex_dat, truth = "obs", estimate = "pred", na.rm = FALSE)[[".estimate"]], 1),
    round(hydroGOF::pbias(obs = ex_dat$obs, sim = ex_dat$pred), 1)
  )

  expect_equal(
    round(pbias(ex_dat, truth = "obs", estimate = "pred_na", na.rm = TRUE)[[".estimate"]], 1),
    round(hydroGOF::pbias(obs = ex_dat$obs[not_na], sim = ex_dat$pred[not_na]), 1)
  )
})
...
test_that("Result interpretation is returned", {
  ...
  expect_equal(
    pbias(ex_dat, truth = "obs", estimate = "pred", performance = TRUE)[[".estimate"]],
    "Poor"
  )
})
